...
pub mod debug {
    ...
    pub fn update_debug_cursor<T: TypePath + Send + Sync>(
        ...
        for ray in sources.iter().filter_map(|s| s.ray) {
            ...
            gizmos.ray(ray.origin, *ray.direction, tailwind::BLUE_500);
            gizmos.sphere(ray.origin, orientation, 0.1, tailwind::BLUE_500);
        }
        ...
        for (is_first, intersection) in sources.iter().flat_map(|m| {
            ...
            let color = match is_first {
                true => tailwind::GREEN_500,
                false => tailwind::PINK_500,
            };
